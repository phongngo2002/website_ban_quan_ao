                ...
                <div class="w-full">
                    <div class="header-cart-total w-full p-tb-40">
                        Tổng: @if( isset($carts->voucher)  && isset($carts))
                            <del class="totalPrice1" style="font-weight: bold;"
                            >{{number_format($carts->totalPrice, 0, ',', '.')}}</del>
                            <sup class="text-danger">-{{$carts->voucher->discount}}%</sup>
                            <br>
                            <span class="delPrice"
                            >{{number_format($carts->totalPrice - ($carts->totalPrice * ($carts->voucher->discount / 100)), 0, ',', '.')}} VNĐ</span>

                        @elseif($carts && empty($carts->voucher))
                            <span class="delPrice"
                            >{{number_format($carts->totalPrice, 0, ',', '.')}} VNĐ</span>
                        @else
                            <span class="delPrice"
                            >0 VNĐ</span>
                        @endif
                        ...
                        <div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
                            ...
                            <div class="flex-w flex-t bor12 p-b-13">
								...
								<span class="mtext-110">
                                    @if($carts->voucher)
                                        <del class="totalPrice1" style="font-weight: bold;"
                                        >{{$carts->voucher ? number_format($carts->totalPrice, 0, ',', '.') : '' }}</del>
                                        <sup class="text-danger">-{{$carts->voucher->discount}}%</sup>
                                        <br>
                                        <span class="delPrice"
                                        >{{number_format($carts->totalPrice - ($carts->totalPrice * ($carts->voucher->discount / 100)), 0, ',', '.')}} VNĐ</span>
                                ...
                                <div class="size-209 p-t-1">
                                    @if($carts->voucher)
                                        <span class="delPrice1"
                                              data-price="{{$carts->totalPrice - ($carts->totalPrice * ($carts->voucher->discount / 100)) + 13000}}"
                                        >{{number_format($carts->totalPrice - ($carts->totalPrice * ($carts->voucher->discount / 100)) + 13000, 0, ',', '.')}} VNĐ</span>
                                    @else
                                        <span class="delPrice1"
